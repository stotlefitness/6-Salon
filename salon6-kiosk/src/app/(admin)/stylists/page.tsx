  ...
  return (
    <main className="flex min-h-screen flex-col gap-4 bg-white p-8 text-zinc-900">
      <div className="space-y-2">
        ...
        <h1 className="text-3xl font-semibold">Stylists</h1>
        <p className="text-sm text-zinc-600">
          Manage stylists and their basic availability. Salon-scoped by RLS.
        </p>
      </div>

      {!canEdit ? (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Only owners/managers can change stylist records. Your role: {staff.role}.
        </div>
      ) : (
        <div className="rounded-lg border border-dashed border-zinc-300 bg-zinc-50 p-6 text-sm text-zinc-700">
          Stylists table will render here with salon_id filter.
        </div>
      )}
    </main>
  );
